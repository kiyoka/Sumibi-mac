...
待機中のEnterはまだ入力先へ渡していないため、そのキーだけを理由に応答を破棄しない。一方、マウス操作などによる実際の入力先・選択範囲の変更は別に検出し、古い結果や待機入力を別の欄へ適用しない。入力先変更時の回復方針、タイムアウト時間と待ち行列の容量は未確定とする。
...
最大文字数は仕様で別途決定する。設定未保存時のOFF、ON時の前後取得、取得不能・片側のみ取得、OFFへの変更後のBYOK再試行を実装時の確認対象とする。

## 8. 候補ウィンドウ

候補要求は「第一候補のみ」と「第二候補以降の追加取得」に分ける。iOS版の`ConversionCandidateMode`と追加候補取得処理を設計の参考にし、実装は共有しない。追加要求には保持している元の原文と現在の変換結果を渡す。追加取得件数は別途決定し、iOS版の件数を自動的に確定仕様とはしない。
