...
最初に、直前に確定した変換を同じカーソル位置で再選択できるか判定する。対象文字列・範囲・入力先に変更がなく、安全に置換できる場合は候補ウィンドウを開く。以下は新しい追跡文字列を変換する場合の流れであり、候補ウィンドウを開くたびにこの経路でLLMを再実行するものではない。候補を初回に取得するか、再度の操作時に追加取得するかは未確定である。
