...
- `Prototype/build.sh`: 開発用の`.app`を`.build/prototype/`に作り、アドホック署名する。公証・配布用の署名ではない。

macOS 26以上とXcodeのSwift環境で`swift test`、`sh Prototype/build.sh`を実行する。試す場合は生成された`.app`を`~/Library/Input Methods/`へコピーし、入力ソースに追加する。入力ソースがすぐに表示されない場合は、ログアウト・再ログイン後に確認する。ログアウトはユーザー自身の判断で行い、試作スクリプトからは行わない。入力ソースは自動選択しない。
...
| 項目 | 結果 |
| --- | --- |
| ビルドとアドホック署名 | 成功。`plutil -lint`と`codesign --verify`も成功 |
...
| 入力ソース登録 | `TISRegisterInputSource`は`noErr`を返した。ただし、macOSの入力ソース一覧と全入力ソース照会にはまだ表示されていない |
...
入力ソースの一覧更新は、アプリのInfo.plistに入力モード・アイコンを補い再登録した後も即時には反映されなかった。ログアウト・再ログインで変わるか未検証である。
