...
| メッセージ(iMessage)の送信欄で変換してからEnter | 送信は1回だけ。送られた内容は「おはよう」のみ(原文の混入・二重送信なし) |
| メッセージの送信欄で変換せずEnter | 送信は1回だけ。内容は`Arigatou`(頭文字の大文字化はiMessage側の自動整形) |
...
チャット系アプリはメッセージ(iMessage)で確認した。送信は1回だけで、原文の混入・二重送信はない。

なお、iMessageは確定した文字列の頭文字を自動的に大文字へ変える(`arigatou` → `Arigatou`)。入力先が確定後の文字列を書き換える例であり、試作の追加候補要求はアンカー(記録した置換位置と文字列)の一致を確認してから置換するため、この場合は置換を行わずに安全側へ倒れる。製品では、こうした自動整形を行う入力先での追加候補の扱いを決める必要がある。

### まだ確認していないこと

1. 60秒タイムアウトの実測(模擬失敗と同じ経路で、遅延だけが異なる)。
2. 1,000文字の境界は自動テストのみで、実アプリでは未確認。
3. 公証・配布経路。

待機中の制御キーの再生と、TextEditでのUndo粒度は、Issue #4の範囲では受け入れる判断とした(下の「現時点で分かった制限」を参照)。

macOSや入力先アプリが`IMKTextInput`の範囲取得・置換・Undo履歴をどう扱うかには差異があり得るため、アプリ横断の試験は引き続き必要である。

## 現時点で分かった制限

`IMKInputController.handle(_:client:)`から通常のEnterは原文を確定して`false`を返せるため、同じキーイベントを入力先へ渡す構成にしている。メモでは改行が1回だけ入り、メッセージ(iMessage)の送信欄では送信が1回だけとなることを確認した。いずれも原文の欠落・重複はなかった。

一度IMEが消費して待ち行列に保管したEnter・Backspace・`Control + J`を、後から元のキーイベントとして入力先アプリに渡す公開`IMKTextInput` APIは確認できなかった。状態管理は入力順を保持するが、試作アダプターは遅延キーを勝手に改行や削除へ読み替えず、適用不能なキーをIMEメニューに表示する。

**Issue #4ではこの状態を受け入れる**(2026-09-22、利用者の判断)。待機中の制御キーは入力順に記録するが再生はしない。イベント合成による再生は権限・アプリ互換性・誤操作のリスクがあるため、必要になった時点で別途仕様を決める。
...
TextEditでは、原文の確定と応答後の置換が1つのUndoにまとめられるため、原文の状態を経由できない。Undoの粒度は入力先アプリの方針であり、公開APIからは区切りを入れられないため、**Issue #4ではこのアプリ差を受け入れる**(2026-09-22、利用者の判断)。利用者自身が前に打った文字が消えないことは確認済みで、そこが安全上の線引きとなる。
