...
| 実アプリへの入力 | 初回の人手試験で、文字を1字入力するたびに前の1字を上書きする不具合を確認。入力先の識別処理を一度修正し、自動操作では`ohayou`の蓄積を確認したが、物理入力では再発したため識別処理自体を除去した。物理キーボードによる再確認が必要 |
...
AppleのSDKヘッダーでは`IMKServer`が入力セッションごとに`IMKInputController`を作成すると説明されている。そのため、コントローラー内で不安定な入力先識別子を重ねて比較する処理を除去した。非同期応答の誤挿入防止は、コントローラー単位の状態に加え、要求ID、選択範囲、対象文字列の一致で検証する。物理入力での未確定文字列と`Control + J`は再確認が必要である。
