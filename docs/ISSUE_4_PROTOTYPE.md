...
| 実アプリへの入力 | 当初は1字入力するたびに前の1字を上書きする不具合が出た。原因はmacOS 27のIMKが消費したキーごとに入力セッションを終了することと判明し(下の節を参照)、状態をプロセス共有へ移して解消した。メモ(Notes)での物理キーボードにより、文字の蓄積、初回変換、候補窓、キーボードとマウスでの候補選択、Undo、Enter、応答待ち中の入力、模擬失敗、入力先変更時の誤挿入防止を確認した(次表)。TextEditでの一連の動作と、60秒タイムアウトの実測は未確認 |
...
1. TextEditやチャット系アプリなど、メモ以外での一連の動作(候補窓、Undo、送信欄でのEnter)。
...
確定方式とUndo履歴の関係は実測で判明した。未確定文字列(`setMarkedText`)から`insertText`で確定すると、原文はUndo履歴に入らず、Undoでは原文へ戻せない。**原文をまず通常の文字として確定し、応答が返ってからその範囲を置換する**方式に変えることで、メモでは1回のUndoで原文へ戻った。アプリ差異は残るため、他アプリでの確認は必要である。
...
- 対処として、**入力先へ無害な書き込み(空文字の`insertText`)を明示的に行い、次のセッションを作らせる**(`pokeClient(_:)`)。応答待ち中にキーを溜めたときと、追加候補を要求したときに呼ぶ。これにより上記の行き止まりは解消した。
